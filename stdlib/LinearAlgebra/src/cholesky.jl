...
function cholesky!(A::RealHermSymComplexHerm, ::Val{false}=Val(false); check::Bool = true)
    if A.uplo == 'U'
        ...
        C = Cholesky(CU.data, 'U', info)
    else
        CL, info = _chol!(A.data, LowerTriangular)
        C = Cholesky(CL.data, 'L', info)
    end
    check && (info == 0 || throw(PosDefException(info)))
    return C
end
...
the factorization was successful, or else throw an error.
...
When `check = true` the matrix will be checked to have full rank, and make sure
the factorization was successful, or else throw an error.
"""
cholesky(A::Union{StridedMatrix,RealHermSymComplexHerm{<:Real,<:StridedMatrix}},
    ::Val{true}; tol = 0.0, check::Bool = true) =
